...
const getAlarmAudioContext = (): AudioContext => {
...
const playReminderSequence = async (label: string) => {
    onStateChange(MochiState.SPEAKING, `Nhắc nhở: ${label}`);
    await speak(`Đã đến giờ ${label}`);
    const soundSource = await createReminderSound();
    soundSource.start();
    soundSource.stop(getAlarmAudioContext().currentTime + 1.5);
    setTimeout(() => onStateChange(MochiState.IDLE, "Mochi đang nghỉ ngơi..."), 2000);
};

const playAlarmSequence = (label: string, id: number) => {
    onStateChange(MochiState.ALARM_RINGING, label);
    ...
    const stop = () => {
        ...
        onStateChange(MochiState.IDLE, "Báo thức đã tắt.");
    };
