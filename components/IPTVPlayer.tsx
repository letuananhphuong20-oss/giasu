...
export const IPTVPlayer: React.FC<IPTVPlayerProps> = ({ channel, isExternallyPaused, channels, onChannelSelect, onClose, isTvFullscreen, onToggleFullscreen }) => {
    ...
    return (
        <div ref={playerContainerRef} className="relative w-full h-full bg-black flex items-center justify-center animate-fade-in rounded-lg overflow-hidden shadow-2xl border border-gray-800">
            <style>{`
            ...
            <div className={`absolute top-0 left-0 right-0 p-4 flex items-center gap-3 transition-opacity duration-300 ${isControlsVisible ? 'opacity-100' : 'opacity-0'}`}>
                {channel.logo && <img src={channel.logo} alt={channel.name} className="h-10 w-auto max-w-[6rem] object-contain rounded-md bg-black/50 p-1" />}
                <h2 className="text-lg font-bold text-white text-shadow">{channel.name}</h2>
                <button onClick={onClose} className="control-btn ml-auto" aria-label="Đóng trình phát">
                    <CloseIcon />
                </button>
            </div>

            {/* Controls */}
            <div className={`absolute bottom-0 left-0 right-0 p-3 flex items-center justify-between gap-4 transition-opacity duration-300 ${isControlsVisible ? 'opacity-100' : 'opacity-0'}`}>
